...
(function () {
  ...
  function send() {
    ...
    /* Тот же дефект, что закрыт 25.08 у поля множественного выбора, только на шагах
       с ручным вводом (обороты, «Я знаю значение»): там рядом с полем стоят «Пропустить»
       и «Назад», а отправку делает не они, а стрелка ввода. Кнопки убирают себя сами
       только по нажатию на них же, поэтому оставались висеть между вопросом и ответом
       покупателя и оставались нажимаемыми. Замер в браузере 25.08: DIV.chips
       «ПропуститьНазад» 66×37 px между репликами «Какие обороты…» и «140»,
       elementFromPoint отдаёт саму кнопку, нажатие уводит подбор на шаг назад уже
       после ответа. Приём тот же, что в buildTopics() и в обработчике «.ms-go». */
    var stale = els.body.querySelectorAll('.chips');
